...
for i in range(rng_per, n):
    if i == rng_per - 1:
        rng[i] = rng_qty * sum(abs(closes[j] - closes[j-1]) for j in range(1, rng_per)) / rng_per
    else:
        rng[i] = rng[i-1] + alpha * (rng_qty * abs(closes[i] - closes[i-1]) - rng[i-1])
